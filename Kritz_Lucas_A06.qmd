...
[https://github.com/lucaskritz/Kritz_Lucas_201_A06.git](https://github.com/lucaskritz/Kritz_Lucas_201_A06.git)
...
![Git Proof](github_setup_png.png)
...
# for loop cycles through each list by name, adds an item called 'height' = the value in 'heights' vector that matches name
for (player in names(player_info)) {
  player_info[[player]]$height <- unname(heights[player]) # unname function used because R includes the name in the new list item by default
}
print(player_info)
```
...
ppg_list = list()
for (player in names(player_info)){
  # iterates through each player to calculate ppg value
  ppg = (player_info[[player]]$points/player_info[[player]]$games_played)
  # assigns ppg to correct player name in ppg_list
  ppg_list[[player]] = ppg
}
print(ppg_list)
```
...
rounded_ppg = sapply(ppg_list, rounder)
print(rounded_ppg)
...
rounder = function(x){
  num <- nchar(x)
  if (num == 2) {
    return(round(x, -1))  #Nearest tens
  } else if (num == 3) {
    return(round(x, -2))  #Nearest hundreds
  } else if (num == 4) {
    return(round(x, -3))  #Nearest thousands
  } else {
    return(x) 
  }
}
apply(mat_rand, c(1, 2), rounder)
```
...
```{r}
avg_temp = function(x){
cols <- as.numeric(x[3:length(x)])  # converts the cells in the proper columns to numeric
  return(mean(cols, na.rm = TRUE))
}
col_avg = apply(max_temps, 2, avg_temp)
col_avg = col_avg[3:length(col_avg)] # adjusts col_avg so it is just the month rows
print(col_avg)
```
...
```{r}
CO_data = subset(max_temps, state == 'CO')
OH_data = subset(max_temps, state == 'OH')
VA_data = subset(max_temps, state == 'VA')
```
...
```{r}
# finds median values for all years
med_vals = apply(CO_data[, -c(1, 2)], 1, median)

max_num = which.max(med_vals) # number of years after first year + 1 that max occured
max_year = min(CO_data$year + max_num - 1)
max_med = med_vals[max_num] # finds max median given by max_num

min_num = which.min(med_vals) # number of years after first year + 1 that min occured
min_year = min(CO_data$year + min_num - 1)
min_med = med_vals[min_num] # finds min median given by min_num

print(paste(
  "The maximum median annual temperature of", max_med, 
  "occurred in", max_year, 
  "and the minimum median annual temperature of", min_med, 
...
digit_sum = function(x){
  if(x < 10){ # base case - if number is a single digit
    return(x)
  } else{
    last_dig = x %% 10 # gets last digit
    other_dig = x %/% 10 # removes last digit using integer div
    return(last_dig + digit_sum(other_dig)) # recursive function - continues until one digit
  }
}
digit_sum(935068)
```
