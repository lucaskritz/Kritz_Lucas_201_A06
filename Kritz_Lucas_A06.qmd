...
```{r}
temp_cols = CO_data[, -c(1, 2)]
min = min(temp_cols)
max = max(temp_cols)

max_row = which(temp_cols == max)
max_year = CO_data$year[max]
print(max_year)
print(maxes)
print(paste("The maximum median annual temperature of ))
```
...
digit_sum = function(x){
  if(x < 10){ # base case - if number is a single digit
    ...
    return(last_dig + digit_sum(other_dig)) # recursive function - continues until one digit left
  }
}
digit_sum(935068)
```
